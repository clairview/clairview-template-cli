import {readFields, readRelations} from '@clairview/sdk'
...
/**
 * Extract relations from the Clairview instance
 */
...
export default async function extractRelations(dir: string) {
  try {
    ...
    // Filter out relations where the collection starts with 'clairview_' && the field is not within the customFields array
    .filter(
      (i: any) =>
        !i.collection.startsWith('clairview_', 0)
            || customFields.some(
