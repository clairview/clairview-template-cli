...
export default async function apply(dir: string) {
  ...
  if (!isTemplateOk) {
    ux.error('The template is missing the collections, fields, or relations files. Older templates are not supported in v0.4 of clairview-template-cli. Try using v0.3 to load older templates npx clairview-template-cli@0.3 apply or extract the template using latest version before applying. Exiting...')
  }
