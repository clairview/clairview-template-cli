...
import {getClairviewToken, getClairviewUrl} from '../lib/utils/auth'
...
async function getTemplate() {
  ...
  if (templateType.templateType === 'community') {
    ...
    try {
      const {dir} = await downloadTemplate('github:clairviews/clairview-templates', {
        dir: downloadDir,
        force: true,
        preferOffline: true,
      })
  ...
  if (templateType.templateType === 'clairview-plus') {
    openUrl('https://clairview.io/plus?utm_source=clairview-template-cli&utm_content=apply-command')
    ux.log('Redirecting to Clairview website.')
    ux.exit(0)
  }
...
export default class ApplyCommand extends Command {
  static description = 'Apply a template to a blank Clairview instance.'

  static examples = ['$ clairview-template-cli apply']
  ...
  public async run(): Promise<void> {
    ...
    const clairviewUrl = await getClairviewUrl()
    await getClairviewToken(clairviewUrl)

    ux.log(separator)

    // Run load script
    ux.log(
      `Applying template - ${chosenTemplate.template.templateName} to ${clairviewUrl}`,
    )
